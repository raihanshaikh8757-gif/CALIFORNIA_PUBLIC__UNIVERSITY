    ...
    <header className="sticky top-0 z-50 border-b border-[#0B1F3A] shadow-lg">
      ...
      {menuOpen ? (
        <div className="fixed inset-0 z-[60] md:hidden">
          ...
          />
          <aside className="absolute right-0 top-0 h-full w-[82%] max-w-[320px] border-l border-white/15 bg-[#0B1F3A] p-4 text-white shadow-2xl">
            <div className="mb-4 flex items-center justify-between">
              <p className="text-sm font-bold tracking-wide">Menu</p>
              <button
                aria-label="Close menu"
                onClick={() => setMenuOpen(false)}
                className="rounded-md border border-white/25 p-1.5"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
